...
import { ScrollReveal, SplitText } from "@/components/site/animations";
...
function Process() {
  ...
  return (
    <>
      <PageHero
        eyebrow="The Process"
        title="From waste to resource, in seven traceable steps."
        lead="A unified operating model designed for compliance, transparency and recovery efficiency at industrial scale."
        image={logistics}
      />
      ...
      <section className="py-28">
        <div className="container-tight">
          {STEPS.map((s, i) => (
            <ProcessRow key={s._id} step={s} index={i} />
          ))}
        </div>
      </section>
    </>
...
function ProcessRow({ step, index }: { step: ProcessStep; index: number }) {
  ...
  const numberColor = useTransform(
    scrollYProgress,
    [0, 0.45, 1],
    ["color-mix(in oklab, var(--color-muted-foreground) 50%, transparent)", "var(--color-eco)", "var(--color-eco)"],
  );
  const numberX = useTransform(scrollYProgress, [0, 0.45], [-40, 0]);
  ...
  return (
    <motion.div
      ...
      className="grid md:grid-cols-12 gap-8 py-10 relative group"
    >
      {/* Animated progress underline */}
      <motion.div
        aria-hidden
        style={{ scaleX: lineScale, transformOrigin: "0% 50%" }}
        className="absolute inset-x-0 top-0 h-px bg-[color:var(--color-eco)]"
      />
      {index === 0 && <span className="absolute inset-x-0 top-0 h-px bg-border" aria-hidden />}
      <motion.div
        style={{ color: numberColor, x: numberX }}
        className="md:col-span-2 display text-5xl md:text-6xl tabular-nums"
      >
        {String(index + 1).padStart(2, "0")}
      </motion.div>
      <div className="md:col-span-4">
        <h3 className="display text-3xl">
          <SplitText as="span" mode="word">{step.title}</SplitText>
        </h3>
      </div>
