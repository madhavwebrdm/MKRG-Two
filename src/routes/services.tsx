          ...
          {SERVICES.map((s, i) => {
            ...
            <Reveal key={s._id} delay={(i % 3) * 80}>
              <article className="group rounded-2xl bg-card border border-border p-8 h-full hover:border-[color:var(--color-eco)]/60 hover:shadow-[var(--shadow-card)] transition-all">
                <Icon className="h-8 w-8 text-[color:var(--color-eco)]" />
                <h3 className="display mt-8 text-2xl">{s.title}</h3>
                <p className="mt-3 text-sm text-muted-foreground leading-relaxed">{s.description}</p>
                <ul className="mt-6 space-y-2 text-sm">
                  {(s.bullets ?? []).map((bi) => <li key={bi} className="flex gap-3 text-muted-foreground"><span className="mt-2 h-1 w-1 rounded-full bg-[color:var(--color-eco)]" />{bi}</li>)}
                </ul>
